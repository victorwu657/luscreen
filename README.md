...
*   **多样化截图**：
    ...
    *   **全屏截图**：一键捕捉整个屏幕。
    *   **窗口捕捉**：适合快速截取指定应用窗口内容。
    *   **滚动长截图**：智能识别滚动窗口，自动拼接生成长图，轻松截取长网页、文档或聊天记录。
...
*   **手绘风格编辑器 (Hand-drawn Style)**：
    ...
    *   **独立画板模式**：不仅能编辑截图，还能直接打开本地图片文件进行标注和处理，随时记录想法与创意。
*   **精准控制**：
...
### 🔍 3. 实用辅助功能
*   **OCR 文字识别**：截图后一键识别图中文字，适合提取报错信息、代码片段、文档内容等；支持复制到剪贴板，提升整理效率。
*   **智能剪贴板**：自动保存复制过的文字与图片，支持历史回溯、搜索与一键粘贴，适合高频跨应用复制场景。
*   **本地优先的数据安全**：官网强调剪贴板历史仅保存在本地电脑，便于兼顾效率与隐私。
*   **全局快捷键**：
    *   `Ctrl+F1`：开始录制
    *   `Ctrl+F2`：暂停/继续录制
    *   `Ctrl+F3`：停止录制
    *   `Ctrl+L`：呼出主菜单
    *   支持用户自定义快捷键，适应个人习惯。
*   **面向多种场景**：适合教育培训、办公协作、Bug 反馈、内容创作、游戏精彩时刻记录等使用场景。
